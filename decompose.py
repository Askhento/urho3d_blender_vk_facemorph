    ...
    for tLodLevel in tLodLevels:
        for i, triangle in enumerate(tLodLevel.triangleList):
            ...
            if d == 0:
                ...
                invalidUV = True
                # print(f"wrong uv ind {i} {vertex1.uv}, {vertex2.uv}, {vertex3.uv} ")
                continue
...
def DecomposeMesh(scene, meshObj, tData, tOptions, errorsMem):
    ...
    verticesMyMap = tData.verticesMyMap # !!!!!
    ...
    # Check if the mesh has UV data, this is done to real mesh not bmesh
    orig_uv = None
    orig_uv2 = None
    # Search the UV layers which name ends in "_UV1" or "_UV2"  
    for uvLayer in meshData.uv_layers:
        if uvLayer.name.endswith("_UV") or uvLayer.name.endswith("_UV1"):
            orig_uv = uvLayer.data
    ...
    if tOptions.vkFace:
        ...
        if not orig_uv:
            orig_uv = meshData.uv_layers.new(name="UVMap_UV1")
            # simple hack so the area of uv is not zero 
            for loop in meshData.loops:
                l_ind = loop.index
                l_total = len(meshData.loops)

                l_ind_norm = l_ind / l_total

                wiggleAng = 2.0 * pi * (l_ind_norm)
                wiggleU = cos(wiggleAng) * 0.5 + 0.5
                wiggleV = sin(wiggleAng) * 0.5 + 0.5

                orig_uv.data[l_ind].uv = (
                    wiggleU, wiggleV
                )
                
        if not orig_uv2:
            orig_uv2 = meshData.uv_layers.new(name="UVMap_UV2")
    ...
    # print(f'num tris : {len(mesh.loop_triangles)}')
    

    print(f'VK face is { "ON!" if tOptions.vkFace else "off"}' )

    
        
    for tri in (mesh.polygons if tOptions.vkFace else mesh.loop_triangles):
        ...
        tLodLevel.vertexIndices  =  [vert.index for vert in mesh.vertices] #!!!
        ...
        for i, vertexIndex in enumerate(tri.vertices):
            ...
            # print(f'v_ind : {vertexIndex}')

            loopIndex = (tri.loop_indices[i] if tOptions.vkFace else tri.loops[i])
            ...
            if mesh.use_auto_smooth and not tOptions.vkFace:
            ...
            if tOptions.vkFace:
                tVertex.tangent = Vector((1.0, 1.0, 1.0, 1.0))
            ...
            #     print(f't_v_ind : {tVertexIndex}, co : {tVertex.pos} \n')
                
            # Create triangles
            if i == 2:
                triangle = (tempList[0], tempList[2], tempList[1])
                if tOptions.vkFace:
                    triangle = (tempList[0], tempList[1], tempList[2])
